...
if ( $('#attendance-form').length > 0 ) {
    $(document).on('submit', '#attendance-form', function(e) {
        ...
        $.ajax({
            ...
            async    : true,
            cache    : false,
            url      : baseurl + "company_one/?case=AttendanceProcessHandler",
            data     : form.serialize(),
            success  : function(result) {
                if ( result.code == 0 ) {
                    form[0].reset();
...
$(document).ready(function() {
    ...
    if ( $('#employees').length > 0 ) {
        ...
        $('#employees tbody').on('click', '.editEmp', function(e) {
            ...
            $.ajax({
                ...
                async    : true,
                cache    : false,
                url      : baseurl + "company_one/?case=GetEmployeeByID",
                data     : 'emp_code=' + data[0],
                success  : function(result) {
                    if ( result.code == 0 ) {
        ...
        $('#holidays tbody').on('click', '.deleteHoliday', function(e) {
            ...
            if ( conf ) {
                ...
                $.ajax({
                    ...
                    async    : true,
                    cache    : false,
                    url      : baseurl + "company_one/?case=DeleteHolidayByID",
                    data     : 'id=' + data[0],
                    success  : function(result) {
                        if ( result.code == 0 ) {
                            $.notify({
    ...
    if ( $('#payheads').length > 0 ) {
        ...
        $('#payheads tbody').on('click', '.editPayheads', function(e) {
            ...
            $.ajax({
                ...
                async    : true,
                cache    : false,
                url      : baseurl + "company_one/?case=GetPayheadByID",
                data     : 'id=' + data[0],
                success  : function(result) {
                    if ( result.code == 0 ) {
    ...
    if ( $('#profile-form').length > 0 ) {
        $('#profile-form').on('submit', function(e) {
            ...
            $.ajax({
                ...
                async    : true,
                cache    : false,
                url      : baseurl + "company_one/?case=EditProfileByID",
                data     : form.serialize(),
                success  : function(result) {
                    if ( result.code == 0 ) {
                        $.notify({
    ...
    if ( $('#password-form').length > 0 ) {
        $('#password-form').on('submit', function(e) {
            ...
            $.ajax({
                ...
                async    : true,
                cache    : false,
                url      : baseurl + "company_one/?case=EditLoginDataByID",
                data     : form.serialize(),
                success  : function(result) {
                    if ( result.code == 0 ) {
                        form[0].reset();
    ...
    if ( $('#myleaves').length > 0 ) {
        var myleave = $('#myleaves').DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": baseurl + "company_one/?case=LoadingMyLeaves",
            "columnDefs": [{
                "targets": 0,
                "className": "dt-center"
            }]
...
function sendPaySlipByMail(emp_code, month) {
    $.ajax({
        ...
        async    : true,
        cache    : false,
        url      : baseurl + "company_one/?case=SendPaySlipByMail",
