<?php
include(dirname(dirname(__FILE__)) . '/tsi_config.php');
    ...
    if ( !empty($code) && !empty($password) ) {
	    ...
	    if ( $adminCheck ) {
	        if ( mysqli_num_rows($adminCheck) == 1 ) {
	            ...
	            $result['result'] = BASE_URL . 'dashboard_ts/';
			    $result['code'] = 0;
	        } else {
...
function LoadingAttendance() {
	...
	$sql  = "SELECT `id`, `employee_id`, `date`, `num_hr` FROM `attendance` GROUP BY `employee_id`, `date`";
	...
	$sql .= " FROM `" . DB_PREFIX . "employees` AS `emp`, `attendance` AS `att` WHERE `emp`.`emp_code` = `att`.`emp_code`";
...
function LoadingSalaries() {
	...
	$requestData = $_REQUEST;
		...
		while ( $row = mysqli_fetch_assoc($query) ) {
			...
			$nestedData[] = '<button type="button" class="btn btn-success btn-xs" onclick="openInNewTab(\'' . BASE_URL . 'pay-salary-at/' . $row['emp_code'] . '/' . date('F',strtotime($row['pay_month'])) . '/' . str_replace(', ', '-', $row['pay_month']) . '.\');"><i class="fa fa-download"></i></button>';
...
function InsertEmployee() {
    ...
    $gsis = stripslashes($_POST['gsis']);
   	$date = date('Y-m-d');
	$insertHoliday = mysqli_query($db, "INSERT INTO `" . DB_PREFIX . "employees`(`emp_code`,`first_name`, `last_name`,`dob`,`gender`,`merital_status`,`nationality`,`address`,`city`,`state`,`country`,`email`,`mobile`,`middle_name`,`position`,`department`,`branch`,`gsis`,`sss`,`tin`,`pagibig`,`pagibig`)
	 VALUES ('$emp_id', '$first_name','$last_name','$dob','$gender','$merital_status','$nationality','$address','$city','$state','$country','$email','$contact','$mid_name','$position','$dept','$branch','$gsis','$sss','$tin','$pagbig','$date')");
		    if ( $insertHoliday ) {
		        $result['result'] = 'Record has been successfully inserted.';
		        $result['code'] = 0;
		    } else {
